...
namespace Rumi.BrigadierForLethalCompany.API.ArgumentTypes.Selectors.Options
{
    /// <summary>
    ...
    public abstract class SelectorOption
    {
        ...
        {
            ReflectionManager.onRefresh += Refresh;
            Refresh();
        }

        static void Refresh()
        {
            List<(Type type, string name, bool onlyEntity)> list = [];
