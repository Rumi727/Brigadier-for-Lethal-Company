...
{
    public class CoordinatesArgumentType<T> : IArgumentType<CoordinatesArgumentValue<T>[]> where T : struct, IComparable<T>
    {
        ...
        protected internal CoordinatesArgumentType(T? minimum, T? maximum, Func<IStringReader, T> readFunc, int argumentCount, int localArgumentCount = 0)
        {
        ...
        public CoordinatesArgumentValue<T>[] Parse(IStringReader reader)
        {
        ...
        public override bool Equals(object? o) => o is CoordinatesArgumentType<T> arg && Equals(minimum, arg.minimum) && Equals(maximum, arg.maximum);
