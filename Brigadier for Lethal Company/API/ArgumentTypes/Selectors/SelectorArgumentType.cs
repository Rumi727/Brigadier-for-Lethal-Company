...
{
    public class SelectorArgumentType : IArgumentType<SelectorArgumentValue>
    {
        readonly bool onlyPlayer = false;
        readonly bool limit = false;

        protected internal SelectorArgumentType(bool onlyPlayer = false, bool limit = false)
        {
            this.onlyPlayer = onlyPlayer;
            this.limit = limit;
        }

        public SelectorArgumentValue Parse(IStringReader reader)
        {
            if (!reader.CanRead())
        ...
        public SelectorOption[] GetOptions(IStringReader reader, bool existEntity = false, bool isLimitedType = false)
        {
            ...
            if (reader.CanRead() && !char.IsWhiteSpace(reader.Peek()))
            {
                ...
                result = [.. options.OrderBy(static x => x.sort)];
            }
